...
def browse_files(file_type='', is_dir=False):
    ...
    def real_browse_files(func):
        """Browse through every file & folder before running function"""

        def wrapper(*args, **kwargs):
            for i in settings.BASE_DIR.glob(f'**/*{file_type}'):
                if not any(directory in str(i.absolute()) for directory in settings.IGNORE_LIST):
                    if is_dir == True:
                        if i.is_dir():
                            func(i, *args, **kwargs)
                    else: 
...
def main():
    # copy_files('Photos', ('.jpg', '.png'))
    copy_files('GIFs', ('.gif'))

    # remove_files(file_contains='(1)')
    # list_files()
